...
public class Problem_1269 {
    public static int numWays(int steps, int arrLen) {
        ...
        for(int i = 1; i <= steps; i++){
            for(int j = 0; j <= maxCol; j++){
                dp[i][j] = dp[i-1][j];
                if(j-1 >= 0){
                    dp[i][j] = (dp[i][j] + dp[i-1][j-1]) % DIVISOR;
                }
                if(j+1 <= maxCol){
                    dp[i][j] = (dp[i][j] + dp[i-1][j+1]) % DIVISOR;
                }
    ...
    //dp 的每一行只和上一行有关  可调整以节省空间
    public static int numWaysSec(int steps, int arrLen) {
        int DIVISOR = (int)Math.pow(10,9) + 7;
        int maxCol = Math.min(arrLen - 1, steps);
        ...
        for(int i = 1; i <= steps; i++){
            ...
            for (int j = 0; j <= maxCol; j++) {
                dpNext[j] = dp[j];
                if (j - 1 >= 0) {
                    dpNext[j] = (dpNext[j] + dp[j - 1]) % DIVISOR;
                }
                if (j + 1 <= maxCol) {
                    dpNext[j] = (dpNext[j] + dp[j + 1]) % DIVISOR;
                }
    ...
    public static void main(String[] args) {
        System.out.println(numWaysSec(4,2));
